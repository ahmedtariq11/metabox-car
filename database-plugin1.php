...
add_action( 'admin_init', 'my_adminq' );

function my_adminq() {
    add_meta_box( 
        'car_review_meta_box',
        'Informazioni Auto',
        'display_car_review_meta_box1',
        'auto',
        'normal',
        'high'
    );
...
function display_car_review_meta_box1() {
    ?>
    <table>
        <tr>
            <td style="width: 50%">UIID</td>
            <td><input type="text" size="40" name="garage" value="<?php echo get_post_meta( get_the_ID(), 'id', true ); ?>" /></td>
        </tr>
        <tr>
...
function techiepress_check_for_similar_meta_ids() {
 ...
 $id_arrays_in_cpt[] = get_post_meta( get_the_content(), 'content', true );
 $id_arrays_in_cpt[] = get_post_meta( get_the_title(), 'title', true );
    }
